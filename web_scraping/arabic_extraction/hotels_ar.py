...
while True:
    ...
    try:
        ...
        for row in rows:
            ...
            if len(cols) >= 5:
                data.append({
                    'hotel_ar': cols[0].text.strip(),
                    'hotel_en': cols[1].text.strip(),
                    'governorate': cols[2].text.strip(),
...
# Saved to a new CSV file to keep your data organized
pd.DataFrame(data).to_csv("hotel_ar.csv", index=False, encoding='utf-8-sig')
print(f"Extraction complete! Saved {len(data)} hotels.")